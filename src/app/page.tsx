...
//BKT Engine Mathematical Logic
interface BKTParams {
  pL0: number; // Iniital mastery probability
...
function getAdjustedParams(base: BKTParams, difficulty: 'Easy' | 'Medium' | 'Hard'): BKTParams {
  switch (difficulty) {
    case 'Easy':
      return { ...base, pG: 0.30, pS: 0.05 };
    case 'Hard':
      return { ...base, pG: 0.10, pS: 0.20 };
    case 'Medium':
    default:
      return base;
  }
...
// Application Data and Interfaces
...
const INITIAL_SPREADSHEETS: ClassSpreadsheet[] = [
  {
    ...
    data: [
      { id: 'ST-10A01', name: 'Alex Brown', grade: 'Grade 10A', quadraticsMastery: 0.45, polynomialsMastery: 0.31, systemsMastery: 0.40, weakestTopic: 'Polynomial Factoring', status: 'Critical Intervention', lastActive: '10 mins ago' },
      ...
      { id: 'ST-10A08', name: 'Sam Greenwood', grade: 'Grade 10A', quadraticsMastery: 0.92, polynomialsMastery: 0.90, systemsMastery: 0.88, weakestTopic: 'Systems of Equations', status: 'Mastered', lastActive: '3 hrs ago' },
      { id: 'ST-10A09', name: 'Sergio Arribas', grade: 'Grade 10A', quadraticsMastery: 0.45, polynomialsMastery: 0.50, systemsMastery: 0.42, weakestTopic: 'Systems of Equations', status: 'Review Recommended', lastActive: 'Yesterday' },
      ...
      { id: 'ST-10A12', name: 'Tom Bischof', grade: 'Grade 10A', quadraticsMastery: 0.72, polynomialsMastery: 0.75, systemsMastery: 0.70, weakestTopic: 'Systems of Equations', status: 'Review Recommended', lastActive: '6 hrs ago' },
      { id: 'ST-10A13', name: 'Elliot Anderson', grade: 'Grade 10A', quadraticsMastery: 0.55, polynomialsMastery: 0.60, systemsMastery: 0.58, weakestTopic: 'Quadratic Equations', status: 'Review Recommended', lastActive: '1 day ago' },
      { id: 'ST-10A14', name: 'Anthony Gordon', grade: 'Grade 10A', quadraticsMastery: 0.30, polynomialsMastery: 0.35, systemsMastery: 0.31, weakestTopic: 'Quadratic Equations', status: 'Critical Intervention', lastActive: '3 days ago' },
      ...
      { id: 'ST-10A22', name: 'Morgan Gibbs-White', grade: 'Grade 10A', quadraticsMastery: 0.70, polynomialsMastery: 0.68, systemsMastery: 0.71, weakestTopic: 'Polynomial Factoring', status: 'Review Recommended', lastActive: 'Yesterday' },
      { id: 'ST-10A23', name: 'Emile Smith Rowe', grade: 'Grade 10A', quadraticsMastery: 0.77, polynomialsMastery: 0.80, systemsMastery: 0.74, weakestTopic: 'Systems of Equations', status: 'Mastered', lastActive: '2 hrs ago' },
      ...
      { id: 'ST-10A25', name: 'Jacob Ramsey', grade: 'Grade 10A', quadraticsMastery: 0.82, polynomialsMastery: 0.80, systemsMastery: 0.85, weakestTopic: 'Polynomial Factoring', status: 'Mastered', lastActive: '3 hrs ago' },
...
export default function App() {
  ...
  // Selected Student Modal / View state for teacher dashboard action
  const [selectedStudentForView, setSelectedStudentForView] = useState<SpreadsheetStudent | null>(null);
  ...
  const studyProgressPercentage =
    totalParagraphsCount > 0
      ? Math.round((completedParagraphsCount / totalParagraphsCount) * 100)
      : 0;
  ...
  //Cohort Add and Delete Handlers
  ...
  const handleAddStudentToSheet = (sheetId: string, e: React.FormEvent) => {
    ...
    setSpreadsheets((prev) =>
      prev.map((sheet) =>
        sheet.id === sheetId ? { ...sheet, data: [...sheet.data, newStudent] } : sheet
      )
    );
        ...
        return {
          ...
          data: sheet.data.map((student) => {
            ...
            if (field === 'quadraticsMastery' || field === 'polynomialsMastery' || field === 'systemsMastery') {
              ...
              if (avg < 0.4) updated.status = 'Critical Intervention';
              else if (avg < 0.75) updated.status = 'Review Recommended';
              else updated.status = 'Mastered';
            }
            return updated;
          }),
        };
  ...
  const handleRemoveStudentFromSheet = (sheetId: string, studentId: string) => {
    setSpreadsheets((prev) =>
      prev.map((sheet) =>
        sheet.id === sheetId
          ? { ...sheet, data: sheet.data.filter((s) => s.id !== studentId) }
          : sheet
      )
    );
  };
  ...
  const handleGenerateBriefing = () => {
    ...
    const briefingText = `Parent & Admin Progress Briefing for ${foundStudent.name} (${foundStudent.grade}):\n\n` +
      ...
      `Core strengths are observed in ${strengthsStr}, where response trajectories indicate reliable application and low slip frequency from a starting baseline mastery of 25%. ` +
  ...
  //Teacher Assessment Form Helper Methods
  const handleQuestionAmountChange = (newAmount: number) => {
    ...
    setManualQuestions((prev) => {
      if (targetAmount > prev.length) {
        const added: ManualQuestionEntry[] = Array.from({ length: targetAmount - prev.length }).map(
          (_, i) => ({
  ...
  const updateQuestionField = (
    index: number,
    field: keyof ManualQuestionEntry,
  ...
  const handleAnalyzeTeacherInput = (e: React.FormEvent) => {
    ...
    const topicLogs: Record<string, ResponseLog[]> = {
      quadratics: [],
      polynomials: [],
      systems: [],
    };
    ...
    const recommendations: TeacherRecommendation[] = rawRecommendations.map((rec) => {
      ...
      return {
        ...rec,
        currentMastery: topicResults[rec.topicId],
      };
    });
    setTeacherAnalysis({
      studentName,
  ...
  //Aggregated data for primary Teacher Dashboard
  const allStudents = spreadsheets.flatMap((s) => s.data);
  const totalStudentsCount = allStudents.length || 28;
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold text-slate-900">
            Teacher Analytics Dashboard & BKT Engine
          </h1>
          ...
          <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200 text-sm font-medium shadow-xs">
            <button
              ...
              className={`px-4 py-2 rounded-md transition-all ${
                role === 'teacher' ? 'bg-white text-slate-900 shadow-sm font-bold' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              Teacher
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-md transition-all ${
                role === 'student' ? 'bg-white text-slate-900 shadow-sm font-bold' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              Student
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-md transition-all ${
                role === 'mentor' ? 'bg-white text-slate-900 shadow-sm font-bold' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              Mentor
            </button>
        ...
        <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          <div className="flex border-b border-slate-200 bg-white px-4 rounded-lg border overflow-x-auto shadow-xs">
            <button
              ...
              className={`py-4 px-6 text-base font-semibold border-b-2 transition-colors whitespace-nowrap ${
          ...
          {/* TAB 1: Centerpiece Teacher Dashboard */}
          {teacherTab === 'dashboard' && (
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
                  ...
                  <div className="p-4 bg-sky-50 rounded-lg border border-sky-100">
                    ...
                    <span className="block text-3xl font-extrabold text-sky-900 mt-1">67%</span>
                  </div>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between items-center mb-1 text-sm font-semibold">
                      <span className="text-slate-800">Quadratic Equations</span>
                      <span className="font-mono text-sky-700 font-bold">78%</span>
                    </div>
                    <div className="w-full bg-slate-100 rounded-full h-3.5 border border-slate-200 overflow-hidden">
                      <div className="bg-sky-600 h-full rounded-full transition-all duration-500" style={{ width: '78%' }} />
                    </div>
                  </div>

                  <div>
                    <div className="flex justify-between items-center mb-1 text-sm font-semibold">
                      <span className="text-slate-800">Polynomial Factoring</span>
                      <span className="font-mono text-sky-700 font-bold">61%</span>
                    </div>
                    <div className="w-full bg-slate-100 rounded-full h-3.5 border border-slate-200 overflow-hidden">
                      <div className="bg-sky-600 h-full rounded-full transition-all duration-500" style={{ width: '61%' }} />
                    </div>
                  </div>

                  <div>
                    <div className="flex justify-between items-center mb-1 text-sm font-semibold">
                      <span className="text-slate-800">Systems of Equations</span>
                      <span className="font-mono text-sky-700 font-bold">69%</span>
                    </div>
                    <div className="w-full bg-slate-100 rounded-full h-3.5 border border-slate-200 overflow-hidden">
                      <div className="bg-sky-600 h-full rounded-full transition-all duration-500" style={{ width: '69%' }} />
                    </div>
                  </div>
                </div>
              </div>
                      ...
                      {[...criticalInterventionStudents, ...reviewRecommendedStudents].slice(0, 10).map((student) => {
                        ...
                        return (
                          <tr key={student.id} className="hover:bg-slate-50 transition-colors">
                            <td className="p-3.5 font-semibold text-slate-900">{student.name}</td>
                            ...
                            <td className="p-3.5">
                              <span
                                className={`inline-block px-2.5 py-1 text-xs font-semibold rounded border ${
                                  student.status === 'Critical Intervention'
                            ...
                            <td className="p-3.5 text-right">
                              <button
                                ...
                                className="px-3 py-1.5 bg-sky-600 hover:bg-sky-700 text-white rounded text-xs font-semibold transition-colors shadow-xs"
                              >
                                View
                              </button>
                            </td>
                  ...
                  <div className="bg-white border border-slate-200 rounded-xl max-w-lg w-full p-6 space-y-4 shadow-xl">
                    <div className="flex justify-between items-start border-b border-slate-100 pb-3">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{selectedStudentForView.grade} | ID: {selectedStudentForView.id}</p>
                      </div>
                      <button
                    ...
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between py-1 border-b border-slate-100">
                        <span className="text-slate-500">Overall Flag Status:</span>
                        <span className="font-semibold text-rose-600">{selectedStudentForView.status}</span>
                      </div>
                      <div className="flex justify-between py-1 border-b border-slate-100">
                        <span className="text-slate-500">Weakest Focus Topic:</span>
                        <span className="font-semibold text-slate-800">{selectedStudentForView.weakestTopic}</span>
                      </div>
                      <div className="space-y-2 pt-2">
                        <span className="block text-xs font-bold text-slate-500 uppercase tracking-wider">Detailed Topic Mastery</span>
                        <div className="flex justify-between items-center text-xs">
                          <span>Quadratic Equations</span>
                          <span className="font-mono font-bold">{Math.round(selectedStudentForView.quadraticsMastery * 100)}%</span>
                        </div>
                        <div className="flex justify-between items-center text-xs">
                          <span>Polynomial Factoring</span>
                          <span className="font-mono font-bold">{Math.round(selectedStudentForView.polynomialsMastery * 100)}%</span>
                        </div>
                        <div className="flex justify-between items-center text-xs">
                          <span>Systems of Equations</span>
                          <span className="font-mono font-bold">{Math.round(selectedStudentForView.systemsMastery * 100)}%</span>
                        </div>
                      </div>
                    </div>
                    <div className="pt-4 flex justify-end">
                      <button
                        onClick={() => setSelectedStudentForView(null)}
                        className="px-4 py-2 bg-slate-800 text-white rounded-md text-xs font-semibold hover:bg-slate-900 transition-colors"
                      >
              ...
              {expandedSheetId === null ? (
                <div className="space-y-6">
                  ...
                  <form
                    ...
                    className="bg-white border border-slate-200 rounded-lg p-5 space-y-4 shadow-xs"
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="md:col-span-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            type="submit"
                            className="px-5 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-md text-xs font-semibold transition-colors shrink-0 shadow-xs"
                          >
                            + Create Cohort
                          </button>
                        </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-white border border-slate-200 rounded-lg p-5 flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">
                          Select a cohort spreadsheet to add or edit student records, or remove an unused cohort.
                        </p>
                      </div>
                    ...
                    {spreadsheets.length === 0 ? (
                      <div className="bg-white border border-slate-200 rounded-lg p-12 text-center text-slate-400 text-sm">
                        No grade cohorts defined. Use the creation panel above to add a new cohort.
                      </div>
                    ) : (
                        ...
                        {spreadsheets.map((sheet) => {
                          ...
                          return (
                            <div
                              ...
                              className={`p-4 rounded-lg border text-left cursor-pointer transition-all flex flex-col justify-between h-36 shadow-xs group relative ${
                                isPopulated
                            ...
                            >
                              ...
                              <button
                                onClick={(e) => handleDeleteCohort(sheet.id, e)}
                                title="Delete cohort"
                                className="absolute top-3 right-3 text-slate-400 hover:text-red-600 p-1 rounded hover:bg-red-50 transition-colors"
                              >
                              ...
                              <div className="flex justify-between items-center border-t border-slate-100 pt-2 mt-2">
                                <span className="text-xs font-medium text-slate-500">
                                  {isPopulated ? `${studentCount} student${studentCount === 1 ? '' : 's'}` : 'Empty'}
                                </span>
                      ...
                      <div className="bg-white border border-slate-200 rounded-lg p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                          <button
                            ...
                            className="text-xs font-semibold text-sky-600 hover:text-sky-800 mb-2 flex items-center gap-1"
                          >
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={(e) => handleDeleteCohort(activeExpandedSheet.id, e)}
                          ...
                          <span className="text-xs bg-sky-50 text-sky-700 px-3 py-1.5 rounded border border-sky-200 font-semibold">
                            {activeExpandedSheet.data.length} Student Roster Count
                          </span>
                        </div>
                      </div>

                      <form
                        onSubmit={(e) => handleAddStudentToSheet(activeExpandedSheet.id, e)}
                        className="bg-sky-50/60 border border-sky-200 rounded-lg p-4 flex flex-col sm:flex-row items-center gap-3"
                      >
                        ...
                        <div className="sm:self-end w-full sm:w-auto">
                          <button
                            type="submit"
                            className="w-full sm:w-auto px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md text-xs font-semibold shadow-xs transition-colors"
                          >
                            + Add Student
                          </button>
                        </div>
                      ...
                      <div className="bg-white border border-slate-200 rounded-lg p-6">
                        {activeExpandedSheet.data.length === 0 ? (
                          <div className="text-center py-12 text-slate-400 text-sm">
                            No students in this spreadsheet yet. Use the form above to add students.
                          </div>
                        ) : (
                          <div className="overflow-x-auto border border-slate-200 rounded-lg">
                            <table className="w-full text-left border-collapse">
                              ...
                              <tbody className="divide-y divide-slate-200 text-sm">
                                {activeExpandedSheet.data.map((row) => (
                                  <tr key={row.id} className="hover:bg-slate-50/80 transition-colors">
                                      ...
                                      <input
                                        ...
                                        onChange={(e) =>
                                          handleUpdateStudentField(
                                            activeExpandedSheet.id,
                                            row.id,
                                            'name',
                                            e.target.value
                                          )
                                        }
                                        className="w-full px-2 py-1 text-sm bg-transparent border border-transparent hover:border-slate-300 focus:border-sky-500 focus:bg-white rounded font-semibold text-slate-900 transition-colors"
                                      />
                                    </td>
                                    ...
                                    <td className="p-3.5">
                                      <span className={`inline-block px-2.5 py-1 text-xs font-semibold rounded border ${row.status === 'Mastered' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : row.status === 'Review Recommended' ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
                                        {row.status}
                                      </span>
                                    </td>
                                    <td className="p-3.5 text-right">
          ...
          {teacherTab === 'assessment' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <form
                onSubmit={handleAnalyzeTeacherInput}
                className="lg:col-span-5 bg-white border border-slate-200 rounded-lg p-6 space-y-5 shadow-xs"
              >
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">
                    Amount of Questions Assessed ({manualQuestions.length})
                  </label>
                  <input
                    type="number"
                    min={1}
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                  <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 block">
                    Question Breakdown & Evaluation
                  </span>
                  {manualQuestions.map((q, idx) => (
                    <div
                      key={q.id}
                      className="p-3 border border-slate-200 rounded-md bg-slate-50 space-y-2"
                    >
                      <div className="flex justify-between items-center text-xs font-bold text-slate-700">
                        ...
                        <label className="flex items-center cursor-pointer space-x-2">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              updateQuestionField(idx, 'isCorrect', e.target.value === 'true')
                            }
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="block text-xxs font-medium text-slate-500 mb-0.5">
                            Topic Type
                          </label>
                          <select
                            value={q.topicId}
                            onChange={(e) => updateQuestionField(idx, 'topicId', e.target.value)}
                ...
                <button
                  type="submit"
                  className="w-full py-3 bg-sky-600 hover:bg-sky-700 text-white text-sm font-semibold rounded-md transition-colors shadow-xs"
                >
                ...
                {teacherAnalysis ? (
                      ...
                      <div className="flex justify-between items-start border-b border-slate-100 pb-3">
                        <div>
                          ...
                          <p className="text-sm text-slate-600 mt-0.5">
                            Student: <span className="font-semibold">{teacherAnalysis.studentName}</span>{' '}
                            ({teacherAnalysis.studentGrade})
                          </p>
                        </div>
                        <span className="text-xs bg-sky-50 text-sky-700 px-3 py-1 rounded border border-sky-200 font-semibold">
                          {manualQuestions.length} Questions Processed
                        </span>
                      </div>
                        ...
                        {TOPICS.map((t) => {
                          ...
                          return (
                            <div key={t.id} className="p-3 bg-slate-50 rounded border border-slate-100">
                              ...
                              <span
                                className={`block text-base font-bold font-mono mt-1 ${
                                  score !== null ? 'text-slate-800' : 'text-slate-400 italic text-xs'
                                }`}
                              >
                    ...
                    <div className="bg-white border border-slate-200 rounded-lg p-6">
                      ...
                      <div className="space-y-4">
                        {teacherAnalysis.recommendations.map((rec) => (
                          <div
                            key={rec.topicId}
                            className="p-4 border border-slate-200 rounded-lg flex flex-col sm:flex-row justify-between sm:items-center gap-3 bg-slate-50/50"
                          >
                            <div>
                            ...
                            <div className="shrink-0 text-left sm:text-right">
                              <span
                                className={`inline-block text-xs font-semibold px-2.5 py-1 rounded border ${
                                  rec.status === 'Not Assessed'
                                    ? 'bg-slate-100 border-slate-300 text-slate-600'
                                    : rec.status === 'Mastered'
                                    ? 'bg-emerald-50 border-emerald-200 text-emerald-700'
                                    : rec.status === 'Review Recommended'
                                    ? 'bg-amber-50 border-amber-200 text-amber-700'
                              ...
                              <div className="text-xs font-mono text-slate-500 mt-1">
                                {rec.currentMastery !== null
                                  ? `Skill Confidence = ${(rec.currentMastery * 100).toFixed(1)}%`
                  ...
                  <div className="bg-white border border-slate-200 rounded-lg p-12 text-center text-slate-500">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      Fill out student evaluation details on the left and submit to view personalized paths.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white border border-slate-200 rounded-lg p-6 space-y-6">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-0.5">
                    Instantly parse telemetry history into a plain-language summary paragraph for parent-teacher conferences.
                  </p>
                </div>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-5 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      ...
                      <select
                        value={synthesisGradeFilter}
                        onChange={(e) => setSynthesisGradeFilter(e.target.value)}
                        className="w-full px-3 py-2 text-sm bg-white border border-slate-300 rounded-md focus:outline-none focus:border-sky-500"
                      >
                  ...
                  <div>
                    ...
                    <select
                      value={synthesisStudentId}
                      onChange={(e) => setSynthesisStudentId(e.target.value)}
                      className="w-full px-3 py-2 text-sm bg-white border border-slate-300 rounded-md focus:outline-none focus:border-sky-500"
                    >
                      {filteredStudents.length === 0 ? (
                        ...
                        filteredStudents.map((stu) => {
                          const avg = (
                            ((stu.quadraticsMastery + stu.polynomialsMastery + stu.systemsMastery) / 3) *
                            100
                          ).toFixed(1);
                          return (
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-sky-600 hover:bg-sky-700 text-white text-sm font-semibold rounded-md shadow-xs transition-colors flex items-center justify-center gap-2"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <div className="border border-emerald-200 bg-emerald-50/50 rounded-lg p-5 space-y-3">
                    <div className="flex justify-between items-center border-b border-emerald-200 pb-2">
                      <span className="text-xs font-bold uppercase tracking-wider text-emerald-800">
                        Generated Synthesis Paragraph (Ready for Export)
                      </span>
                      <button
                        onClick={() => {
          ...
          <div className="flex justify-between items-center bg-white px-4 rounded-lg border border-slate-200 shadow-xs">
            <div className="flex">
              <button
                ...
                className={`py-4 px-6 text-base font-semibold border-b-2 transition-colors ${
                  activeTab === 'materials'
                    ? 'border-sky-600 text-sky-700'
                    : 'border-transparent text-slate-500 hover:text-slate-800'
                }`}
              ...
              <div className="bg-white border border-slate-200 rounded-lg p-5">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-semibold uppercase tracking-wider text-slate-700">
                    Overall Curriculum Completion Progress
                  </span>
                ...
                <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
                  <div
                    ...
                    style={{ width: `${studyProgressPercentage}%` }}
                  />
                </div>
              </div>
                    ...
                    <div className="space-y-4">
                      {topic.paragraphs.map((paragraph) => {
                        ...
                        return (
                          <div
                            key={paragraph.id}
                            className="border border-slate-200 rounded-lg overflow-hidden bg-slate-50/50 shadow-xs"
                          >
                            <div
                              onClick={() => toggleParagraphExpand(paragraph.id)}
                            ...
                            {isExpanded && (
                              <div className="p-5 bg-slate-50 border-t border-slate-200 text-sm md:text-base text-slate-700 leading-relaxed font-sans">
                                {paragraph.content}
                              </div>
                            )}
                          </div>
                ...
                <nav className="space-y-2">
                  {TOPICS.map((topic) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 text-sm rounded-md font-semibold transition-colors ${
                  ...
                  <div className="bg-white border border-slate-200 rounded-lg p-6 flex flex-col justify-between">
                    {currentQuestion ? (
                      <>
                        <div>
                          <div className="flex justify-between items-center mb-5">
                            ...
                            <span className="text-sm font-bold px-3 py-1 rounded border border-sky-200 bg-sky-50 text-sky-700">
                              Difficulty: {currentQuestion.difficulty}
                            </span>
                          </div>
                          <p className="text-base font-semibold text-slate-900 mb-6 leading-normal">
                          ...
                          <div className="space-y-3">
                            {currentQuestion.options.map((option, idx) => (
                              <button
                                ...
                                className={`w-full text-left px-4 py-3.5 text-sm md:text-base font-medium rounded-lg border transition-colors ${
                                  selectedOption === idx
                      ...
                      <div className="py-16 text-center text-sm font-medium text-slate-500">
                        Topic evaluation complete! All questions answered.
                      </div>
                    )}
                  </div>
                  <div className="bg-white border border-slate-200 rounded-lg p-6">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider">
                        Estimated Mastery: {TOPICS.find((t) => t.id === activeTopicId)?.title}
                      </h3>
        ...
        <div className="max-w-6xl mx-auto p-6 space-y-6">
          <div className="bg-white border border-slate-200 rounded-lg p-6 space-y-6">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">
                Review live help requests from students struggling with specific questions and connect to provide real-time guidance.
              </p>
            </div>
            ...
            {activeHelpSession ? (
                ...
                <div className="flex justify-between items-center border-b border-sky-200 pb-3">
                  <div>
                    <h3 className="text-base font-bold text-sky-950">
                      Live Mentorship Session with {activeHelpSession.studentName}
                    </h3>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => setActiveHelpSession(null)}
                      className="px-3 py-1.5 bg-white border border-sky-300 text-sky-800 rounded text-xs font-semibold hover:bg-sky-100 transition-colors"
                    >
                      Minimize
                    </button>
                  </div>
                ...
                <div className="space-y-3 max-h-60 overflow-y-auto p-3 bg-white border border-slate-200 rounded-md">
                  {chatLog.length === 0 ? (
                    <p className="text-xs text-slate-400 text-center py-4">
                      No messages yet. Send a guidance hint or breakdown to start helping!
                    </p>
                  ) : (
                    chatLog.map((msg, i) => (
                      <div
                        key={i}
                        className={`flex flex-col ${msg.sender === 'mentor' ? 'items-end' : 'items-start'}`}
                      >
                        <span className="text-xxs text-slate-400 mb-0.5">
                          {msg.sender === 'mentor' ? 'You (Mentor)' : activeHelpSession.studentName}
                        </span>
                        <div
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && chatMessage.trim()) {
                        ...
                        setTimeout(() => {
                          setChatLog((prev) => [
                            ...prev,
                            { sender: 'student', text: 'Thanks! That explanation makes much more sense now.' },
                          ]);
                        }, 1000);
                      }
                    }}
              ...
              <div className="space-y-3">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500 block">
                  Incoming Student Help Queue ({mentorRequests.length})
                </span>
                {mentorRequests.length === 0 ? (
                  <p className="text-xs text-slate-400 text-center py-8 bg-slate-50 rounded-lg border border-slate-100">
                    No active peer mentor requests right now.
                  </p>
                ) : (
                  mentorRequests.map((req) => (
                    <div
                      key={req.id}
                      className="p-4 border border-slate-200 rounded-lg bg-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-slate-100/60 transition-colors"
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-bold text-slate-900">{req.studentName}</span>
                          <span className="text-xxs bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded font-mono font-semibold">
                            Mastery: {(req.currentMastery * 100).toFixed(1)}%
                          </span>
